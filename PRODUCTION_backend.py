...
def setup_remotion_project(job_dir, brand_data):
    ...
    composition = f'''import {{Composition, useCurrentFrame, useVideoConfig, interpolate, spring}} from 'remotion';
...
const Video = () => {{
  ...
  return (
    <div style={{{{flex: 1, background: 'linear-gradient(135deg, {brand_data["primary_color"]} 0%, {brand_data["secondary_color"]} 100%)', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', fontSize: 60, color: 'white', fontFamily: 'Arial, sans-serif'}}}}>
      {{frame < 90 && (<div style={{{{transform: `scale(${{logoSpring}})`, opacity: logoOpacity, fontSize: 100, fontWeight: 900, textShadow: '0 0 40px rgba(255,255,255,0.5)', color: 'white', textAlign: 'center', padding: '0 40px'}}}}>{brand_data["name"]}</div>)}}
      {{frame >= 90 && frame < 270 && (<div style={{{{opacity: taglineOpacity, transform: `translateY(${{taglineY}}px)`, fontSize: 50, fontWeight: 700, textAlign: 'center', padding: '0 50px', maxWidth: '90%'}}}}>{brand_data["tagline"]}</div>)}}
      {{frame >= 270 && (<div style={{{{opacity: ctaOpacity, transform: `scale(${{ctaScale}})`, fontSize: 60, fontWeight: 900, color: '{brand_data["accent_color"]}', backgroundColor: 'white', padding: '20px 60px', borderRadius: '50px'}}}}>{brand_data["cta"]}</div>)}}
    </div>
  );
}};
...
def upload_pdf():
    ...
    try:
        ...
        jobs[job_id] = {"created_at": time.time(), "brand": brand_data["name"], "status": "extracted", "brand_data": brand_data}
        return jsonify({"job_id": job_id, "status": "extracted", "brand": brand_data["name"]})
    except Exception as e:
...
if __name__ == '__main__':
    print("Starting Video Backend on port 5000")
